...
### How does Stump determine whether or not a book is allowed?

If a book or a book's series has an age restriction set, Stump will use that age restriction to determine whether or not a user can access it. It is a `less than or equal to` comparison, meaning that if a user has an age restriction set to `13` and a book is rated to `17`, the user will not be able to access the book. If a user has an age restriction set to `17` and a book is rated to `13`, the user will be able to access the book.
...
- If neither the book nor the series has an age restriction set, the book will be considered allowed UNLESS a user has an age restriction with the `restrict_on_unset` option set.
