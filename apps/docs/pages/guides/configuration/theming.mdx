...
For example, the default dark theme for Stump can be found [here](https://github.com/stumpapp/stump/tree/nightly/packages/components/themes/dark.ts). For brevity, the entire theme won't be shown here, but let's say we wanted to make a new theme called "midnight" that is based on the dark theme, but with a darker background:
...
Then we should add it to the main [`ThemeSelect` component](https://github.com/stumpapp/stump/tree/nightly/packages/browser/src/scenes/settings/app/preferences/ThemeSelect.tsx) so that it can be selected:

```tsx filename="scenes/settings/app/preferences/ThemeSelect.tsx" copy
<NativeSelect
	value={theme}
	options={[
...
Note that the `label` is a translation key, so we should add it to the [`locales/en.json` file](https://github.com/stumpapp/stump/tree/nightly/packages/browser/src/i18n/locales/en.json), as well.
...
```tsx filename="scenes/settings/app/preferences/FontSelect.tsx" copy
<ComboBox
