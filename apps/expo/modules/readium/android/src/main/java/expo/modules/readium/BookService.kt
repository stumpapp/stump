...
sealed class BookServiceError : Exception() {
    data class AssetRetrievalFailed(
        val error: AssetRetriever.RetrieveError,
    ) : BookServiceError() {
        override val message: String
            get() = "Failed to retrieve asset: ${error.message}"
    }

    data class PublicationNotFound(
        val bookId: String,
    ) : BookServiceError() {
        override val message: String
    ...
    data class PublicationRestricted(
        val bookId: String,
    ) : BookServiceError() {
        override val message: String
...
class BookService(
     ...
     */
    fun getPublication(bookId: String): Publication? = publications[bookId]
