...
pub struct MediaMetadata {
	...
	/// The (height, width) resolutions of each page in the associated media. This is
	/// generated by the image analysis job and is [None] before being generated.
	#[serde(alias = "PageResolutions", default = "Option::default")]
	pub page_resolutions: Option<PageResolutions>,
...
impl From<media_metadata::Data> for MediaMetadata {
	fn from(metadata: media_metadata::Data) -> Self {
		let page_resolutions = match metadata.page_resolutions() {
			Ok(opt) => opt.map(|data| PageResolutions::from(data.to_owned())),
			Err(_e) => None,
		};
