...
// use crate::{config::get_thumbnails_dir, db::entity::Media};

// use super::{error::FileError, media};

// pub fn get_bytes<P: AsRef<Path>>(path: P) -> Result<Vec<u8>, FileError> {
...
// pub fn webp_from_path<P: AsRef<Path>>(file_path: P) -> Result<Vec<u8>, FileError> {
...
// pub fn jpeg_from_path<P: AsRef<Path>>(file_path: P) -> Result<Vec<u8>, FileError> {
// 	let image = Reader::open(file_path.as_ref())?
// 		.with_guessed_format()?
// 		.decode()?;
...
// pub fn jpeg_from_bytes(bytes: &[u8]) -> Result<Vec<u8>, FileError> {
...
// pub fn webp_from_bytes(bytes: &[u8]) -> Result<Vec<u8>, FileError> {
// 	let image = image::load_from_memory(bytes)?;
...
// pub fn generate_thumbnail(id: &str, path: &str) -> Result<PathBuf, FileError> {
// 	let (_, buf) = media::get_page(path, 1)?;
