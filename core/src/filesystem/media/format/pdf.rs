...
impl PdfProcessor {
	...
	fn convert_to_output_format(
		png_data: &[u8],
		target_format: &SupportedImageFormat,
	) -> Result<(ContentType, Vec<u8>), FileError> {
		match target_format {
			SupportedImageFormat::Png => Ok((ContentType::PNG, png_data.to_vec())),
...
impl FileConverter for PdfProcessor {
	fn to_zip(
		...
		for (idx, image_buf) in converted_pages.into_iter().enumerate() {
			...
			let output_extension = match target_format {
				SupportedImageFormat::Png => "png",
				SupportedImageFormat::Jpeg => "jpg",
