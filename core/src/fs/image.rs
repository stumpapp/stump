...
pub fn generate_thumbnails(media: Vec<media::Data>) -> ProcessFileResult<Vec<PathBuf>> {
	...
	// let tasks: Vec<JoinHandle<ProcessFileResult<PathBuf>>> = media
	// 	.into_iter()
	// 	.map(|m| {
	// 		tokio::task::spawn_blocking(move || {
	// 			generate_thumbnail(m.id.as_str(), &m.path.as_str())
	// 		})
	// 	})
	// 	.collect();
...
pub fn remove_thumbnails(id_list: &[&str]) -> ProcessFileResult<()> {
	for id in id_list {
