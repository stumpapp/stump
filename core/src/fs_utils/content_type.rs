...
/// [`ContentType`] is an enum that represents the HTTP content type supported by Stump
#[allow(non_camel_case_types)]
#[derive(Serialize, Debug, Clone, Copy, PartialEq, Eq, Default, Display, EnumString)]
pub enum ContentType {
	...
	ZIP,
	#[strum(serialize = "application/vnd.comicbook+zip")]
	COMIC_ZIP,
	#[strum(serialize = "application/vnd.rar")]
	RAR,
	#[strum(serialize = "application/vnd.comicbook-rar")]
	COMIC_RAR,
...
impl ContentType {
	...
	pub fn from_bytes_with_fallback(bytes: &[u8], extension: &str) -> ContentType {
		let from_bytes = infer_mime_from_bytes(bytes)
			.map(|mime| mime.parse::<ContentType>().unwrap_or_default());
