...
use super::{scale_height_dimension, scale_width_dimension, ImageProcessorError};
...
impl ImageProcessorOptionsExt for ImageProcessorOptions {
	...
	fn validate(&self) -> Result<(), ImageProcessorError> {
		if let Some(quality) = self.quality {
			if !(0..=100).contains(&quality) {
				return Err(ImageProcessorError::InvalidQuality);
			}
		}
