...
// i had some problems with a pub const serde_json::Value so just shoved it in a OnceLock
static NATIVE_KOBO_RESOURCES: OnceLock<serde_json::Value> = OnceLock::new();

/// returns the native kobo resources, which a device seems to require to re-sync properly.
/// before returning as-is, we need to override the image url fields according for this
/// server instance
pub fn native_kobo_resources() -> &'static serde_json::Value {
	NATIVE_KOBO_RESOURCES.get_or_init(|| {
		serde_json::from_str(NATIVE_KOBO_RESOURCES_JSON)
			.expect("native Kobo resources JSON is malformed")
	})
}
