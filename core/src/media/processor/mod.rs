...
use rar::RarProcessor;
use zip::ZipProcessor;
...
pub use error::MediaProcessorError;
...
pub trait MediaProcessor {
	...
	/// Process the raw metadata bytes of a file, without parsing into a struct
	fn process_metadata_raw(
		&self,
		path: &Path,
	) -> Result<Option<Vec<u8>>, MediaProcessorError>;
...
#[tracing::instrument(err, fields(path = %path.as_ref().display()))]
pub async fn process_metadata_raw(
	path: impl AsRef<Path>,
) -> Result<Option<Vec<u8>>, MediaProcessorError> {
	let path = path.as_ref().to_path_buf();
	// we won't use the config since pdf is not supported so debug() is fine, a bit of a smell
	// but its fine
	let processor = get_processor(&path, &StumpConfig::debug()).await?;
	spawn_blocking(move || processor.process_metadata_raw(&path)).await?
}
