...
You may also trigger a manual sync by selecting the `Attempt Sync` option from the action menu in the local library tab. This will attempt to sync all books with progression, both from device to server and from server to device.
