...
The app supports 4 types of authentication:
...
4. Authless (i.e., no authentication)
...
Please note that if you are configuring an OPDS v1.2 server with login-based authentication, the feed must not be behind a login wall. Unlike Stump's native API or the newer OPDS v2.0, OPDS v1.2 does not have a standardized way to handle login flows.

### OIDC Authentication

<Callout title="Permission required" type="info">
...
When you choose a login-based authentication type for a Stump server that has OIDC configured, you will see a "Login with OIDC" button on the login sheet. This will initiate the OIDC login flow as described in the [OIDC guide](/docs/guides/access-control/oidc).
...
<Callout title="Performance Note" type="warning">
	Basic Auth for OPDS requires the server to verify your password on every request. This involves
	computationally expensive password hashing, which impacts image loading. If you are a heavy OPDS
	user, consider using [Token-based Authentication](#token-based-authentication) with a scoped API
	key for much faster performance
</Callout>
...
<Callout type="info">If there is a server that should be listed here, please let me know!</Callout>

## Authless

You can configure a server to be authless, which would effectively make Stump assume that the URL as-is will be authenticated. The name might feel a little misleading for scenarios where your server _is_ behind auth, but the URL just contains, e.g., an API key.

For servers which support URL-based credentials, like embedding an API key directly in the URL, this is the method you should choose.
