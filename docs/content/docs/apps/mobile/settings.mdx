...
A light and dark theme is available by default. The [ebook reader](/docs/apps/mobile/reading/books#visual-themes) has more options available.

### Accent Color

Customize the app's accent color using a hue and chroma scale picker, which generates a palette of colors with various lightnesses:

- **Hue**: The base color (e.g., orange, blue, green)
- **Chroma Scale**: The intensity/saturation of the palette (0 = desaturated, 1 = fully saturated)

The accent colors affect buttons, selections, and other interactive elements throughout the app

### Language

<Callout type="warning" title="Work in progress">
    Not all areas of the app are fully localized yet, but this will improve over time as the app is developed
</Callout>
...
Quick access to switch between your configured servers. See the [default server](/docs/apps/mobile/servers#default-server) section for more information about server management.
...
You do not need a Stump server to use the app. Any OPDS-compatible server will work for basic catalog browsing and reading.
