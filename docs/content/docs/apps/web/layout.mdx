...
Recently added series. Drag a section by its handle or use the arrow buttons to
change its position. The drag handle also supports the keyboard: press Space to
pick up a section, use the arrow keys to move it, and press Space again to drop it.
...
the saved layout unchanged. **Restore defaults**, followed by **Save**, restores
