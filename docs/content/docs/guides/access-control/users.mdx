...
- **Age restriction**: The _optional_ age restriction of the user. This is used to determine which books the user can access. You may enter a number corresponding to the **maximum** age rating the user can access. For example, if you enter `13` then the user will be able to access books with an age rating of `13` or lower. See the [age restrictions](/docs/guides/access-control/age-restrictions) section of the Access Control guide for more information
...
2. Use the embedded CLI in the Stump server to unlock the user account. See the [CLI](/docs/developer/cli) guide for more information. In general, the command will look like this:
...
If a user has forgotten their password, you will have to use the embedded CLI in the Stump server to reset the user's password. See the [CLI](/docs/developer/cli) guide for more information. In general, the command will look like this:
