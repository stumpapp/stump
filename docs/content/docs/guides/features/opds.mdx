---
title: OPDS
description: Compatibility information for OPDS
---
...
Where `your-server` is the domain or IP address of your Stump server, `:10801` is the port (if you are using the default port), and `/baseUrl` if you've put Stump behind a some sort of proxy that includes a path to access it. A few examples just to hopefully help clarify:

- `http://192.168.1.1:10801/opds/v1.2/catalog`
- `https://stump.example.com/opds/v1.2/catalog`
...
For additional information on API keys, see the [API keys](/docs/guides/features/api-keys) guide.
...
- `http://192.168.1.1:10801/opds/v2.0/catalog`
- `https://stump.example.com/opds/v2.0/catalog`

<Callout type="info">
	Stump hasn't implemented `facets` support yet, but it is planned for a future release
</Callout>
