...
Kobo sync is disabled by default. To enable it, you need to set the `STUMP_ENABLE_KOBO_SYNC` environment variable to `true`, or directly set `enable_kobo_sync=true` in the `Stump.toml` configuration file.
