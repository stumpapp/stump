...
The KoReader router is disabled by default. To enable it, you need to set the `STUMP_ENABLE_KOREADER_SYNC` environment variable to `true`, or directly set `enable_koreader_sync=true` in the `Stump.toml` configuration file.
...
KoReader supports two methods for matching books:

1. A hash of the file
2. The name of the file

Stump **only supports hash-based book matching** at this time. This means you will need to ensure Stump generates KoReader-specific hashes for library content for the integration to work.

To enable this feature, be sure the `KoReader-compatible hashes` setting is enabled for a library. For more information, see the [scanner configuration](/docs/guides/fundamentals/scanner) guide.

<Callout>
	To generate hashes on an existing library, you will need to configure a scan to recompute hashes.
	You can do this in the library settings page for scanning-related options
</Callout>
</Step>
...
<Callout type="warn">This step requires the `ACCESS_API_KEYS` user permissions.</Callout>
...
KoReader requires you to login before it can sync progress. This is a bit unintuitive since you are using an API key, but the login step is necessary. You just need to enter any username and password (they are not validated).

<Callout>
	Stump does not currently support anything other than the API key authentication method. If you
	would like to see support for the username/password method, please open a feature request
</Callout>

</Step>

<Step>
### Ensure binary hash is enabled in KoReader

KoReader might default to using a filename-based matching strategy for identifying books with the server. Stump only supports the binary hash method, so you need to ensure that KoReader is set to use this method:

1. Navigate to `Progress sync` > `Document matching method`
2. Select `Binary`
3. Attempt to push progress again (if needed) to verify
...
2. **Progress translation**: The progress data which gets stored is primarily for KoReader's use, with the exception of the percentage. I'd like to add a feature to translate the x-pointer into something useful for the Stump reader so it can pick up where you left off
