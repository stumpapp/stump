...
<Callout emoji="⚠️">
  This functionality allows files to be uploaded to your server, so it should be used with caution, and requires the server owner manually enable it by setting the `enable_upload` configuration variable to `true`.
</Callout>
...
When uploading is not enabled, the routes for uploading will not be attached to your server, and the web UI will not show the upload interface. Attempting to access uploading directly through the API will result in HTTP Error 404.
...
The servr owner can assign these permissions to users on the user management settings page. For more information on that process, see the [user permissions](/guides/access-control/permissions) guide.
...
Users can uploada  `.zip` file containing multiple books in an arbitrary directory structure. The archive will be extracted to the location specified when uploading.
...
Despite these validations, it may be possible to construct input that circumvents this validation. Server owners are encouraged to only enable this functionality if they need it, and to only allow trusted users permission to upload files.
...
This error indicates that the user attempting to upload files does not have the required `upload:files` or `manage:library` permissions.
...
<Callout emoji="📢">
  If you have any questions or encounter issues, please reach out via the [community forums](https://community.stumpapp.dev) or open an [issue](https://github.com/stumpapp/stump/issues/new/choose) on GitHub.
</Callout>
