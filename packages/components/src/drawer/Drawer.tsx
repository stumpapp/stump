...
>(({ className, children, showTopIndicator = true, ...props }, ref) => (
	<DrawerPortal>
		...
		<DrawerPrimitive.Content
			ref={ref}
			className={cn(
				'inset-x-0 bottom-0 mt-24 fixed z-50 flex h-auto flex-col rounded-t-2xl border border-border bg-background outline-none',
				className,
			)}
			{...props}
		>
			{showTopIndicator && (
				<button
					type="button"
					className="mt-4 h-2 w-25 mx-auto scale-[99%] rounded-full bg-muted/40 transition-all duration-200 outline-none hover:scale-100 hover:bg-muted/50 active:scale-[99%]"
				/>
			)}
			{children}
		</DrawerPrimitive.Content>
...
>(({ className, ...props }, ref) => (
	<DrawerPrimitive.Title
		ref={ref}
		className={cn('text-lg font-semibold tracking-tight leading-none text-foreground', className)}
		{...props}
	/>
))
