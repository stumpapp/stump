...
>(({ className, children, showTopIndicator = true, ...props }, ref) => (
	<DrawerPortal>
		...
		<DrawerPrimitive.Content
			ref={ref}
			className={cn(
				'border-edge-subtle fixed inset-x-0 bottom-0 z-50 mt-24 flex h-auto flex-col rounded-t-[10px] border bg-background outline-none',
				className,
			)}
			{...props}
		>
...
>(({ className, ...props }, ref) => (
	<DrawerPrimitive.Title
		ref={ref}
		className={cn(
			'text-foreground-subtle text-lg font-semibold leading-none tracking-tight',
			className,
		)}
		{...props}
	/>
