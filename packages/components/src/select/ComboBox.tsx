		...
		if (onAddOption && filter) {
			return (
				<div className="px-1 pb-1 overflow-hidden">
					<Button
						variant="ghost"
						className="px-3 py-2 text-sm font-normal h-auto w-full justify-start rounded-xl break-all whitespace-normal text-foreground hover:bg-accent hover:text-accent-foreground"
						onClick={() => {
			...
			<Popover open={open} onOpenChange={setOpen}>
				<Popover.Trigger asChild disabled={disabled}>
					<Button
						...
						aria-expanded={open}
						className={cn(
							'gap-1.5 justify-between rounded-4xl border-border bg-input/30 text-foreground outline-none hover:bg-input/50 data-[state=open]:border-ring data-[state=open]:ring-[3px] data-[state=open]:ring-ring/50',
				...
				>
					<Command className="p-0 rounded-none bg-transparent text-popover-foreground">
						{filterable && (
							<>
								<Command.Input
									wrapperClassName="m-1 mb-0 h-9 rounded-xl border-none bg-input/30 px-3"
								...
								{options.map((option) => {
									...
									return (
										<Command.Item
											...
											className={cn(
												'gap-2.5 py-2 pr-8 pl-3 relative w-full rounded-xl transition-all duration-75',
